...
Unless deprecated, each is built and published to Docker Hub by GitHub Actions on every commit to its directory. See each directory's README
for details.

* [openmrs-maven-build](openmrs-maven-build) (`partnersinhealth/openmrs-maven-build`) - **Deprecated**, no longer built or
  published. Maven and JDK 8, with the browsers and libraries needed to build and run tests for OpenMRS modules in CI
